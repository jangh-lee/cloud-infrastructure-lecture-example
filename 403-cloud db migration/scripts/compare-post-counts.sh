...
SELECT COALESCE(GROUP_CONCAT(
  CONCAT_WS(':',
    ...
    CASE
      ...
      WHEN LOWER(TRIM(BOTH CHAR(39) FROM COLUMN_DEFAULT)) IN ('current_timestamp', 'current_timestamp()')
        THEN 'current_timestamp'
      ELSE TRIM(BOTH CHAR(39) FROM COLUMN_DEFAULT)
    END,
    TRIM(REPLACE(LOWER(EXTRA), 'default_generated', '')),
