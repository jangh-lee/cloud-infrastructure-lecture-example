# 302 Auto Scaling Hands-on (1)

!!! info "이번 실습 범위"
    301에서 사용한 **Nginx Target 서버로 내 서버 이미지를 생성**하고, 기존 Load Balancer와 Target Group을 그대로 사용합니다. 이어서 Launch Configuration, Auto Scaling Group, Scaling Policy, Cloud Insight Event Rule을 만듭니다.

- [301 Load Balancer 선행 실습](https://jangh-lee.github.io/cloud-infrastructure-lecture-example/labs/301-load-balancer/)
- [GitHub 302 실습 폴더](https://github.com/jangh-lee/cloud-infrastructure-lecture-example/tree/main/302-auto%20scaling)
...
사용자 또는 Bastion의 지속적인 HTTP 요청
        |
        v
301 Load Balancer :80                 그대로 사용
        |
        v
301 Target Group :80, /healthz        그대로 사용
        |
        v
...
301에서 수동으로 만든 서버 여러 대를 Auto Scaling으로 관리되는 서버 1~3대로 교체합니다. 웹 페이지와 `/status.json`은 그대로이므로 **서버 수만 자동으로 바뀌는 과정**에 집중할 수 있습니다.

## 2. 실습 전 준비

301를 끝낸 상태에서 다음 리소스를 기록합니다.

| 값 | 예시 | 용도 |
| --- | --- | --- |
| 이미지 원본 서버 | 301 Target 서버 한 대 | Step 1에서 내 서버 이미지 생성 |
| Load Balancer | 301에서 만든 LB | 외부 진입점 |
| LB URL | `http://...kr.lb.naverncp.com` | 반복 호출 |
| Target Group | 301에서 만든 TG | ASG 연결과 Health Check |
| VPC / 서버 Subnet | 301 서버와 같은 환경 | ASG 서버 배치 |
| Web ACG | 301 서버에 적용한 ACG | Load Balancer Subnet의 HTTP 허용 |
| Bastion | Load Balancer URL 호출이 가능한 서버 | `hey` 부하 명령 실행 |
| 인증키 | 서버 생성에 사용한 키 | 관리자 비밀번호 확인 |

!!! warning "301 리소스를 삭제하지 않습니다"
    Load Balancer와 Target Group은 302에서도 사용합니다. 301 수동 서버도 새 ASG Target이 Healthy가 될 때까지 유지합니다.

## 3. Step 1 - 기존 Target 서버 이미지 생성

!!! warning "기존 301 서버를 이미 만들어 둔 경우"
    이미지 생성 전에 [301 통합 Init Script](https://jangh-lee.github.io/cloud-infrastructure-lecture-example/labs/301-load-balancer/#init-script)를 Target 서버에서 한 번 다시 실행합니다. 최신 설치기에 HTTP Stress API가 추가되어 있으므로 예전에 만든 서버를 그대로 이미지로 만들면 `/stress`가 동작하지 않습니다.

1. **Services > Compute > Server > Server**로 이동합니다.
2. 301 Load Balancer의 Target으로 사용한 서버 한 대를 선택합니다.
3. **서버 관리 및 설정 변경 > 내 서버 이미지 생성**을 클릭합니다.

| 항목 | 값 |
| --- | --- |
| 이미지 이름 | `lab-asg-web-image-v1` |
| 설명 | `301 load balancer target for auto scaling` |

**Services > Compute > Server > Server Image**에서 `lab-asg-web-image-v1`의 상태가 `생성됨`이 될 때까지 기다립니다.

301 통합 Init Script로 설치한 Nginx, 상태 페이지, HTTP Stress API, `stress-ng`, `htop`, 상태 갱신 timer가 이미지에 포함됩니다. Auto Scaling 서버가 부팅되면 timer가 `/status.json`을 자신의 Hostname과 Private IP로 다시 기록합니다.

### 확인하고 넘어가기

- [ ] 이미지 원본이 301 Load Balancer의 기존 Target 서버입니다.
...
| VPC | 301와 같은 VPC |
| Subnet | 301 웹 서버와 같은 Private Subnet |
| 서버 이름 Prefix | `asg` |
| 최소 용량 | `1` |
| 최대 용량 | `3` |
| 기대 용량 | `1` |
...
2. 301 Target Group에 새 서버가 자동 등록됩니다.
...
ASG 서버가 `Healthy`가 된 뒤에만 301에서 수동 등록한 서버들을 Target Group에서 제거합니다. 서버 자체를 바로 삭제할 필요는 없으며 Target 목록에서만 제거해도 됩니다.
...
기존 301 Hostname이 섞여 나오면 수동 Target이 아직 Target Group에 남아 있는지 확인합니다.
...
!!! danger "HTML 페이지가 출력되면 부하가 아닙니다"
    게시판이나 `LB Demo Node` HTML이 출력되면 현재 Target 서버가 예전 버전이라 `/stress` 요청을 메인 페이지로 처리한 것입니다. [301 통합 Init Script](https://jangh-lee.github.io/cloud-infrastructure-lecture-example/labs/301-load-balancer/#init-script)를 기존 Target 서버에서 다시 실행하고 위 확인 명령을 재실행합니다. 이미 `lab-asg-web-image-v1`을 만들었다면 최신 설치가 반영된 서버로 이미지를 다시 생성해야 새 ASG 서버에도 `/stress`가 포함됩니다.
...
| 확인 위치 | 완료 조건 |
| --- | --- |
| Server Image | 301 Target 서버에서 `lab-asg-web-image-v1` 생성 |
...
| `/stress`가 502 | 301 통합 Init Script 재실행 후 새 이미지 생성 |
| `/stress`가 HTML 반환 | 예전 Nginx 설정이므로 301 통합 Init Script 재실행 후 이미지 재생성 |
| `stressRunning`이 바로 `false` | 최신 301 통합 Init Script 재실행, `journalctl -u lb-demo-stress.service -n 30` 확인 |
...
| 기존 서버 응답이 섞임 | 301 수동 Target을 Target Group에서 제거 |
...
303 게시판 Auto Scaling 심화 실습을 이어서 진행한다면 003의 Web, Backend, DB 리소스는 유지합니다.

## 15. 다음 실습

[303 Auto Scaling Hands-on (2)](https://jangh-lee.github.io/cloud-infrastructure-lecture-example/labs/303-auto-scaling/)에서는 003 게시판의 Web 계층을 Public ALB 뒤에서 Auto Scaling합니다. 이번 302과 달리 고정 Backend·DB 연결과 게시글 데이터 유지까지 함께 확인합니다.
