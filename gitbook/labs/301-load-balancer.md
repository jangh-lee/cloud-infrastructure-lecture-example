...
여러 Ubuntu 웹 노드를 만들고 Load Balancer 뒤에 연결해 헬스체크와 트래픽 분산을 확인합니다. Target 서버에는 302 Auto Scaling 부하 실습에 사용할 패키지도 함께 설치합니다.
...
`stress-ng`는 302에서 CPU 부하를 발생시키고, `htop`은 서버에 직접 접속해 문제를 확인할 때 사용할 수 있는 선택 도구입니다. 통합 Init Script가 두 패키지를 Nginx와 함께 설치하며, 302 기본 부하 실습에는 SSH를 사용하지 않습니다.

`lb-demo-stress.service`는 `127.0.0.1:8081`에서만 실행되고 Nginx의 `/stress` 요청을 받습니다. `www-data`가 임시 파일을 만들 수 있도록 작업 디렉터리는 `/tmp`를 사용합니다. 요청이 반복되어도 서버마다 `stress-ng`를 하나만 실행하며 20초 뒤 자동 종료하므로, 302에서 Load Balancer를 통해 제한된 CPU 부하를 줄 수 있습니다. `/stress-status`에서는 요청을 처리한 Hostname, 부하 실행 여부, 프로세스 ID, 종료 코드와 1분 Load Average를 확인할 수 있습니다.
...
301 실습이 끝나도 다음 리소스는 삭제하지 않습니다.

- Load Balancer
- Target Group
- 이미지 원본으로 사용할 Target 서버 한 대

302 Step 1에서 기존 Target 서버를 선택해 내 서버 이미지를 생성합니다. HTTP Stress API, `stress-ng`, `htop`, Nginx와 상태 갱신 timer는 301 통합 Init Script로 이미 설치되어 있습니다.
