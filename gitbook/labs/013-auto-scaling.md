...
!!! info "이번 실습 범위"
    012에서 만든 **Nginx 서버 이미지, Load Balancer, Target Group을 그대로 사용**합니다. 새로 만드는 것은 Launch Configuration, Auto Scaling Group, Scaling Policy, Cloud Insight Event Rule뿐입니다.
...
| 값 | 예시 | 용도 |
| --- | --- | --- |
| 내 서버 이미지 | `lab-lb-web-image-v1` | Launch Configuration 원본 |
...
이미지가 아직 없다면 012 마지막의 **013용 서버 이미지 준비**부터 수행합니다.

### 시작 전 한 번에 확인

012 이미지 원본 서버에서 실행합니다.
...
- `nginx`와 `lb-demo-status.timer`: `enabled`, `active`
- `/healthz`: `ok`
- `/status.json`: 원본 서버의 Hostname과 Private IP
- `stress-ng`, `htop`: 버전 출력
- Server Image: `lab-lb-web-image-v1`, 상태 `생성됨`

!!! warning "012 리소스를 삭제하지 않습니다"
    Load Balancer와 Target Group은 013에서도 사용합니다. 012 수동 서버도 새 ASG Target이 Healthy가 될 때까지 유지합니다.

## 3. Step 1 - Launch Configuration 생성
...
## 4. Step 2 - Auto Scaling Group 생성
...
## 5. Step 3 - 수동 Target을 ASG Target으로 교체
...
## 7. Step 5 - Cloud Insight Event Rule 연결
...
| 확인 위치 | 완료 조건 |
| --- | --- |
| Server Image | 012의 `lab-lb-web-image-v1` 재사용 |
