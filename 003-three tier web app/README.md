...
`BACKEND_UPSTREAM`은 브라우저에 공개하는 값이 아니라 Web Nginx가 API를 전달할 내부 주소입니다. 003과 303 모두 고정 Backend Private IP를 사용합니다. 303에서는 Public ALB가 여러 Web 서버 앞에 추가됩니다.
...
`DB_ALLOWED_HOST`에는 고정 Backend의 Private IP를 입력합니다. 303에서도 Auto Scaling 대상은 Web 계층이므로 Backend와 DB 연결값은 바뀌지 않습니다.
...
`users`는 게시판 회원·관리자 계정이고 `posts.author_name`은 게시글에 저장되는 작성자 표시 이름입니다. 실제 MariaDB 접속 계정과 허용 호스트, 인증 방식, `board_service` 권한은 DB 서버에서 관리자용 SQL로 확인합니다.
